...
public class Board {
	...
	public boolean isValidPosition(int row, int col) {
		// removed the equals signs from row and col compared to well.
		// return row >= 0 && row < well.length && col >= 0 && col < well[0].length;

		// Blocks stack at the bottom of screen
		return row >= 0 && row <= Constants.BOARD_HEIGHT - 4 
			&& col >= 0 && col <= Constants.BOARD_WIDTH - 1;
	...
	public void deleteRow(int n) {
		for (int row = n; row > 0; row--) {//Shifts rows above in deletion process
			for (int col = 0; col < Constants.BOARD_WIDTH; col++) {
				well[row][col] = well[row - 1][col]; //Copies blocks down after removal

			}
		}
		for (int col = 0; col < Constants.BOARD_WIDTH; col++) {
			well[0][col] = false; //Sets top row to empty
		}
	}
